...
float my_sample_directional_soft_shadow(texture2D shadow, vec3 pssm_coord, vec2 tex_scale) {
	//find blocker
	...
	mat2 disk_rotation;
	{
		float r = my_quick_hash(gl_FragCoord.xy) * 2.0 * M_PI;
		float sr = sin(r);
		float cr = cos(r);
...
//
float my_sample_directional_pcf_shadow(texture2D shadow, vec2 shadow_pixel_size, vec4 coord) {
	vec2 pos = coord.xy;
	float depth = coord.z;
	...
	m_var.xyz += normal_bias;
		...
		if (sc_use_directional_soft_shadows && directional_lights.data[idx].softshadow_angle > 0) {
			...
			if (depth_z < directional_lights.data[idx].shadow_split_offsets.x) {
				...
				//MG - Removed Mult by 2.0; maybe same?
				shadow = my_sample_directional_soft_shadow(directional_shadow_atlas, pssm_coord.xyz, tex_scale * directional_lights.data[idx].soft_shadow_scale);
				blend_count++;
			}
			...
			if (blend_count < blend_max && depth_z < directional_lights.data[idx].shadow_split_offsets.y) {
				...
				//MG - Removed Mult by 2.0; maybe same?
				float s = my_sample_directional_soft_shadow(directional_shadow_atlas, pssm_coord.xyz, tex_scale * directional_lights.data[idx].soft_shadow_scale);
			...
			//MG - Removed Mult by 2.0; maybe same?
			shadow = my_sample_directional_pcf_shadow(directional_shadow_atlas, directional_shadow_pixel_size * directional_lights.data[idx].soft_shadow_scale * (blur_factor + (1.0 - blur_factor) * float(directional_lights.data[idx].blend_splits)), pssm_coord);
			...
			if (directional_lights.data[idx].blend_splits) {
				...
				//MG - Removed Mult by 2.0; maybe same?
				float shadow2 = my_sample_directional_pcf_shadow(directional_shadow_atlas, directional_shadow_pixel_size * directional_lights.data[idx].soft_shadow_scale * (blur_factor2 + (1.0 - blur_factor2) * float(directional_lights.data[idx].blend_splits)), pssm_coord);
