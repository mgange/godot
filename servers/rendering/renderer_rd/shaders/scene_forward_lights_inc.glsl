...
//START CUSTOM INSERT ABOVE LIGHT COMPUTE
//
//REF:
//https://medium.com/@ShaderError/godot-custom-shader-built-ins-functions-part-2-3-4a1772c12dfe
//https://www.reddit.com/r/godot/comments/13oi9gw/godot_41_stylized_shadows_not_a_post_processing/
//https://www.reddit.com/r/godot/comments/14q4srr/please_help_me_modify_the_engines_source_code_so/
//
//
//SAVE ELSWHERE & REMOVE?
//
float my_quick_hash(vec2 pos) {
	const vec3 magic = vec3(0.06711056f, 0.00583715f, 52.9829189f);
	return fract(magic.z * fract(dot(pos, magic.xy)));
}
float my_sample_directional_soft_shadow(texture2D shadow, vec3 pssm_coord, vec2 tex_scale, float taa_frame_count) {
	//find blocker
	...
	mat2 disk_rotation;
	{
		float r = my_quick_hash(gl_FragCoord.xy + vec2(taa_frame_count * 5.588238)) * 2.0 * M_PI;
		float sr = sin(r);
		float cr = cos(r);
...
//
float my_sample_directional_pcf_shadow(texture2D shadow, vec2 shadow_pixel_size, vec4 coord, float taa_frame_count) {
	vec2 pos = coord.xy;
	float depth = coord.z;
...
void light_process_directional_shadow(uint idx, vec3 vertex, highp vec2 directional_shadow_pixel_size, inout uint shadow0, inout uint shadow1) {
	...
	if (directional_lights.data[idx].shadow_opacity > 0.001) {
		...
		//MG - I *think* normal_interp == geo_normal
	...
	m_var.xyz += normal_bias;
		...
		if (sc_use_directional_soft_shadows() && directional_lights.data[idx].softshadow_angle > 0) {
			...
			if (depth_z < directional_lights.data[idx].shadow_split_offsets.x) {
				...
				//MG
				shadow = my_sample_directional_soft_shadow(directional_shadow_atlas, pssm_coord.xyz, tex_scale * directional_lights.data[idx].soft_shadow_scale, scene_data_block.data.taa_frame_count);
				blend_count++;
			}
			...
			if (blend_count < blend_max && depth_z < directional_lights.data[idx].shadow_split_offsets.y) {
				...
				//MG
				float s = my_sample_directional_soft_shadow(directional_shadow_atlas, pssm_coord.xyz, tex_scale * directional_lights.data[idx].soft_shadow_scale, scene_data_block.data.taa_frame_count);
			...
			//MG
			shadow = my_sample_directional_pcf_shadow(directional_shadow_atlas, directional_shadow_pixel_size * directional_lights.data[idx].soft_shadow_scale * (blur_factor + (1.0 - blur_factor) * float(directional_lights.data[idx].blend_splits)), pssm_coord, scene_data_block.data.taa_frame_count);
			...
			if (directional_lights.data[idx].blend_splits) {
				...
				//MG
				float shadow2 = my_sample_directional_pcf_shadow(directional_shadow_atlas, directional_shadow_pixel_size * directional_lights.data[idx].soft_shadow_scale * (blur_factor2 + (1.0 - blur_factor2) * float(directional_lights.data[idx].blend_splits)), pssm_coord, scene_data_block.data.taa_frame_count);
...
//
float sample_directional_shadow(uint idx, vec3 vertex, float shadow_size_mult) {
  uint shadow0 = 0;
  uint shadow1 = 0;
  float shadow = 1.0; // no shadow
  light_process_directional_shadow(idx, vertex, scene_data_block.data.directional_shadow_pixel_size * shadow_size_mult, shadow0, shadow1);
  if (idx < 4) {
...
//
//
//END CUSTOM INSERT ABOVE LIGHT COMPUTE
//
//
